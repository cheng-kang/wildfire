  ...
  data () {
    ...
    const validateOldPassword = (rule, value, callback) => {
      ...
      if (typeof (this.$auth.currentUser.reauthenticate) === 'function') {
        ...
        .catch(() => {
          this.passwordTesting = false
          callback(new Error(_this.$i18next.t('UserSetting.error.password')))
        })
    ...
    const validatePasswordCheck = (rule, value, callback) => {
      if (value !== this.accountForm.newPassword) {
        callback(new Error(_this.$i18next.t('UserSetting.error.passwords_dont_match')))
      } else {
        callback()
      }
    }
    const validatePhotoURL = (rule, value, callback) => {
      ...
      avatar.onerror = () => {
        ...
        this.avatarTesting = false
        callback(new Error(_this.$i18next.t('UserSetting.error.invalid_photo_url')))
      }
    }
    ...
    return {
      ...
      rule: {
        ...
        oldPassword: [
          { required: true, message: _this.$i18next.t('UserSetting.error.empty_old_pwd'), trigger: 'blur' },
          { type: 'string', min: 6, message: _this.$i18next.t('UserSetting.error.password_min_length'), trigger: 'blur' },
          { validator: validateOldPassword, trigger: 'blur' }
        ],
        newPassword: [
          { required: true, message: _this.$i18next.t('UserSetting.error.empty_new_pwd'), trigger: 'blur' },
          { type: 'string', min: 6, message: _this.$i18next.t('UserSetting.error.password_min_length'), trigger: 'blur' }
        ],
        passwordCheck: [
          { required: true, message: _this.$i18next.t('UserSetting.error.empty_confirm_pwd'), trigger: 'blur' },
