(() => {
  ...
  function startWildfire () {
    ...
    loadCSS('../dist/aio/static/wildfire.css')
    let jsList = []
    ...
    jsList.push(`../dist/${databaseProvider}/wildfire.min.js`)
    loadJSSequentially(jsList, () => {
      ...
      new window.Vue({ el: '#wildfire-app' })
    })
  }
  ...
  function initDom () {
    ...
    wildfireThreadDom.innerHTML = `
      <div id="wf-loading-modal" class="wf wf-theme-${theme} animate-flicker">
        ...
        <span>${window.i18next.t('text.poweringWildfire')}</span>
      </div>
      <div id="wildfire-app" v-cloak><wildfire></wildfire></div>
      `
  }
  ...
  function presentErrorMsg (errorCode) {
    const msg = window.i18next === undefined ? errorCode : window.i18next.t(errorCode)
