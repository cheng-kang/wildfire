...
(() => {
      ...
      new window.Vue({
        ...
        mounted() {
          window.$_wildfire_reset = (config, err) => {
            ...
            if (wfLoadingModalEle) wfLoadingModalEle.style.display = 'block';
  ...
  function presentErrorMsg(errorCode) {
    const msg = window.wfI18n.t(errorCode);
    const wfLoadingModal = document.getElementById('wf-loading-modal');
    wfLoadingModal.className = `wf wf-theme-${theme}`; // cancel flicker animation

    const msgSpan = wfLoadingModal.children[1];
